	...
	static readonly codec = packetCodec({
		...
		for (const entry of value.entries) {
			...
			if (value.actions & PlayerInfoUpdateAction.UpdateDisplayName)
				writer.writePrefixedOptional(entry.displayName, displayName => ChatComponentCodec.encode(writer, displayName));
		...
		const entries = reader.readArray(() => {
			...
			if (actions & PlayerInfoUpdateAction.UpdateDisplayName)
				entry.displayName = reader.readPrefixedOptional(() => ChatComponentCodec.decode(reader));
